...
BMPImage* read_bmp(FILE* fp, char** error) //read a bmp image from a file. return newly created image
{
    ...
    int validate;
    BMPImage* bmp_image;
    validate = fread(&header,sizeof(header), 1, fp); //read in all the values for the BMP header
...
BMPImage* crop_bmp(BMPImage* image, int x, int y, int w, int h, char** error)//crop an image starting from coordinate (x,y) w = new width size. h = new height size.
{
    BMPImage* new_image;
